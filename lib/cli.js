...
function printRunSummary(report, paths) {
  ...
  if (report.mode === "preflight" || report.mode === "baseline") {
    ...
    if (summary.errors > 0) {
      console.log("Validation scope: local environment baseline only; no target OpenClaw version was tested.");
    }
  }
  ...
  if (important.length > 0) {
    console.log("");
    const errorTitle =
      summary.errors > 0 && (report.mode === "preflight" || report.mode === "baseline")
        ? "Environment issues to fix first:"
        : "Errors to fix first:";
    console.log(summary.errors > 0 ? errorTitle : "Most important warnings:");
